...
`;
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      const y = Math.min(
        rect.height * scale,
        Math.max(0, (rect.height - (event.clientY - rect.top)) * scale),
      );
    ...
    const renderFrame = (timeMs: number) => {
      ...
      if (mouseDampening > 0 && !prefersReducedMotion) {
    ...
    return () => {
      ...
      const tryCall = (value: unknown, key: string) => {
        if (
          typeof value === "object" &&
          value !== null &&
          key in value &&
          ...
          (value as Record<string, () => void>)[key]();
        }
      };

      tryCall(programRef.current, "remove");
      tryCall(geometryRef.current, "remove");
      tryCall(meshRef.current, "remove");
      tryCall(rendererRef.current, "destroy");
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "gradient-blinds-container size-full overflow-hidden",
        className,
      )}
